                ...
                <div className="h-64 flex items-end justify-between space-x-2">
                  {completionRates.map((rate, i) => (
                    <div key={i} className="flex flex-col items-center">
                      ...
                      <div className="text-xs text-[#64748b] mt-2">{period === "daily" ? `Day ${i + 1}` : period === "weekly" ? `Week ${i + 1}` : period === "monthly" ? `Month ${i + 1}` : `Year ${i + 2020}`}</div>
                    </div>
                  ))}
                </div>
              </CardContent>
                ...
                <div className="flex justify-center space-x-6 mt-4">
                  {Object.keys(subjectPerformance).map((subj) => (
                    <div key={subj} className="flex items-center">
                      <div className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: subj === "Mathematics" ? "#3b82f6" : subj === "Science" ? "#f59e0b" : "#ef4444" }}></div>
              ...
              <CardContent>
                <div className="h-64 flex items-center justify-center">
                  <svg width="220" height="220" viewBox="0 0 220 220">
                    ...
                    <circle cx="110" cy="110" r="80" fill="none" stroke="#3b82f6" strokeWidth="30" strokeDasharray="502.4" strokeDashoffset={502.4 - (gradeDistribution["A"] || 0) * 5.024} transform="rotate(-90 110 110)" />
                    <circle cx="110" cy="110" r="80" fill="none" stroke="#10b981" strokeWidth="30" strokeDasharray="502.4" strokeDashoffset={502.4 - ((gradeDistribution["B"] || 0) + (gradeDistribution["A"] || 0)) * 5.024} transform="rotate(-90 110 110)" />
                    <circle cx="110" cy="110" r="80" fill="none" stroke="#f59e0b" strokeWidth="30" strokeDasharray="502.4" strokeDashoffset={502.4 - ((gradeDistribution["C"] || 0) + (gradeDistribution["B"] || 0) + (gradeDistribution["A"] || 0)) * 5.024} transform="rotate(-90 110 110)" />
                    <circle cx="110" cy="110" r="80" fill="none" stroke="#ef4444" strokeWidth="30" strokeDasharray="502.4" strokeDashoffset={502.4 - ((gradeDistribution["D & F"] || 0) + (gradeDistribution["C"] || 0) + (gradeDistribution["B"] || 0) + (gradeDistribution["A"] || 0)) * 5.024} transform="rotate(-90 110 110)" />
                  </svg>
                </div>
                <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 mt-2">
                  <div className="flex items-center">
                    <div className="w-3 h-3 bg-[#3b82f6] rounded-full mr-2"></div>
                    <span className="text-sm">A ({(gradeDistribution["A"] || 0).toFixed(1)}%)</span>
                  </div>
                  <div className="flex items-center">
                    <div className="w-3 h-3 bg-[#10b981] rounded-full mr-2"></div>
                    <span className="text-sm">B ({(gradeDistribution["B"] || 0).toFixed(1)}%)</span>
                  </div>
                  <div className="flex items-center">
                    <div className="w-3 h-3 bg-[#f59e0b] rounded-full mr-2"></div>
                    <span className="text-sm">C ({(gradeDistribution["C"] || 0).toFixed(1)}%)</span>
                  </div>
                  <div className="flex items-center">
                    <div className="w-3 h-3 bg-[#ef4444] rounded-full mr-2"></div>
                    <span className="text-sm">D & F ({(gradeDistribution["D & F"] || 0).toFixed(1)}%)</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="h-64 relative">
                  <svg className="w-full h-full" viewBox="0 0 800 300">
                    ...
                    <rect x="400" y={250 - (submissionTimeline["After deadline"] || 0) * 2.5} width="60" height={(submissionTimeline["After deadline"] || 0) * 2.5} fill="#ef4444" />
